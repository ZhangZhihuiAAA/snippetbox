...
func (app *application) render(w http.ResponseWriter, 
    ...
    // we are safe to go ahead and write the HTTP status code to 
    // http.ResponseWriter.
    w.WriteHeader(status)

    // Write the contents of the buffer to the http.ResponseWriter. 
    // Note: this is another time where we pass our http.ResponseWriter 
    // to a function that takes an io.Writer.
    buf.WriteTo(w)
}

// newTemplateData returns a pointer to a templateData struct 
// initialized with the current year. Note that we're not using 
// the *http.Request parameter here at the moment, but we will do.
